...
import { useLocation } from 'react-router-dom'; // Detect navigation events
...
const HomePage = () => {
  ...
  // Update displayed products whenever cart changes or when returning to HomePage
  useEffect(() => {
    const syncDisplayedProducts = () => {
    ...
    syncDisplayedProducts();
  }, [cart, location]); // Re-run this effect when `cart` changes or the location changes
    ...
    <div>
      <div className="search-bar">
        ...
        <button onClick={toggleSortByPrice}>
          Сортировать по цене: {sortAscending ? 'Дороже' : 'Дешевле'}
        </button>
      </div>
